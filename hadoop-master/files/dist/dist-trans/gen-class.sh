...
################################################ 
PWD=/root/dist
metamodel=fr.atlanmod.class_.impl.classPackageImpl # transformation metamodel
...
case $key in
    ...
    ;;
    --usage )
   java  -cp ./libs/instantiator/instantiator.jar fr.inria.atlanmod.instantiator.neoEMF.Launcher
        shift
        ;;              
    *)
    ;;
...
echo "Start generating model dag model with $sizeModels vertices"
...
java $JAVA_OPTS -cp ./libs/instantiator/instantiator.jar:$HADOOP_CLASSPATH fr.inria.atlanmod.instantiator.neoEMF.Launcher -c $metamodel -u $suff -n 1 -s $sizeModels -v 0.1 -e $seed -d 8
