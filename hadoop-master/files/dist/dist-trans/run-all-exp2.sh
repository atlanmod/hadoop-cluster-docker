...
#sizeModels=(1000)
declare -a sizeModels=( '100' '1000' '2000' '5000' '1000' )
#sizeModels=(1000)
declare -a mappers=8

#experiments folder
EXP=experiments-data-2
...
for index in `seq 0 $(( mLen-1))`  
	...
	denLen=${#denModels[@]}
	for den_i in `seq 0 $(( denLen-1 ))`
		do
		echo 'generating  a model with average number of vertices equals to: ' ${sizeModels[$index]} 'and density '${denModels[$den_i]} 
		java $JAVA_OPTS -cp ../dist-gen/fr.inria.atlanmod.dag.jar:$HADOOP_CLASSPATH fr.inria.atlanmod.dag.instantiator.Launcher -u $suff -n 1 -s ${sizeModels[$index]} -v 0.1 -e $seed -r ${denModels[$den_i]} 2>/dev/null		
		...
		for i in `seq 1 $iter`
			...
			echo "start running the transformation $fin"
				 ./run.sh -f topoSort.emftvm -s $metamodel -t $metamodel -i $fin -o $fout -m $mappers 2>$EXP/${sizeModels[$index]}/dag-${sizeModels[$index]}-density${denModels[$den_i]}-pass-$i.txt 
			echo "tranformation of model $fout pass $i density ${denModels[$den_i]}finished"
			#disable table  in hbase
			echo "removing table dag_model0_gen0_${sizeModels[$index]}_out" 
