...
echo "transforming model dag/model0/gen0/$sizeModels/"
fin=$suff/class/model0/gen0/$sizeModels
fout=$fin/out/$mappers
		
echo "start running the transformation $fin"
 ./run.sh -f Class2Relational.emftvm -s $s_metamodel -t $t_metamodel -i $fin -o $fout -m $mappers 
echo "tranformation of model $fout pass $i mappers $mappers  finished"
