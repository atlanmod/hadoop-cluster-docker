...
s_metamodel=fr.atlanmod.class_.impl.classPackageImpl # transformation metamodel
t_metamodel=fr.atlanmod.relational.impl.relationalPackageImpl
...
#sizeModels=(1000^6)
declare -a sizeModels=(  '20000' '50000' '100000' ); 
#sizeModels=(1000^6)
declare -a mappers=( '2' '4' '8' )
#experiments folder
...
for index in `seq 0 $(( mLen-1))`  
	...
	for i in `seq 1 $iter`
		...
		java $JAVA_OPTS -cp ./libs/instantiator/instantiator.jar:$HADOOP_CLASSPATH fr.inria.atlanmod.instantiator.neoEMF.Launcher -c $s_metamodel -u $suff -n 1 -s ${sizeModels[$index]} -v 0.1
                echo "transforming model class/model0/gen0/${sizeModels[$index]}/"
                fin=$suff/class/model0/gen0/${sizeModels[$index]}
                fout=$fin/out          
		for map_i in `seq 0 $(( mapLen-1 ))`
			do
			echo "start running the transformation $fin"
				 ./run.sh -f Class2Relational.emftvm -s $s_metamodel -t $t_metamodel -i $fin -o $fout -m ${mappers[$map_i]} -c 2>$EXP/${sizeModels[$index]}/class-${sizeModels[$index]}-mappers${mappers[$map_i]}-random-pass-$i.txt 
			echo "tranformation of model $fout pass $i mappers ${mappers[$map_i]}  finished"
			#disable table  in hbase
	        	echo "disable 'class_model0_gen0_${sizeModels[$index]}_out'" | hbase shell 2>/dev/null
			echo "drop 'class_model0_gen0_${sizeModels[$index]}_out'" | hbase shell 	2>/dev/null

			echo "start running the transformation $fin on greedy mode"
				 ./run.sh -f Class2Relational.emftvm -s $s_metamodel -t $t_metamodel -i $fin -o $fout -m ${mappers[$map_i]} -c -g 2>$EXP/${sizeModels[$index]}/class-${sizeModels[$index]}-mappers${mappers[$map_i]}-greedy-pass-$i.txt 
			echo "tranformation of model $fout pass $i mappers ${mappers[$map_i]}  finished"
			#disable table  in hbase
	        	echo "disable 'class_model0_gen0_${sizeModels[$index]}_out'" | hbase shell 2>/dev/null
			echo "drop 'class_model0_gen0_${sizeModels[$index]}_out'" | hbase shell 	2>/dev/null
		...
		echo "disable 'class_model0_gen0_${sizeModels[$index]}'" | hbase shell 2>/dev/null
                echo "drop 'class_model0_gen0_${sizeModels[$index]}'" | hbase shell 2>/dev/null
	done
done
